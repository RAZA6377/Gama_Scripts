...
vote_machine = {"end": {"last_vote_start_time": 0, "vote_duration": 60,
...
def vote(pb_id, client_id, vote_type):
    ...
    if now < game_started_on + min_game_duration_to_start_vote:
        ...
        return
    if len(voters) == 0:
        ba.internal.chatmessage(f"A {vote_type} Has Started")
    ...
    if pb_id not in voters:
        voters.append(pb_id)
        _ba.screenmessage(f'Successfully Voted Please Ask Other Players To Vote For {vote_type}', transient=True,
                          clients=[client_id])
        if vote_type == 'end':
