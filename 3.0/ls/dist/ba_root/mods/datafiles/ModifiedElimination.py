...
from bascenev1lib.actor.spazbot import SpazBotSet, SpazBotDiedMessage, BomberBot, BomberBotPro, BomberBotProShielded,BrawlerBot, BrawlerBotPro,BrawlerBotProShielded,TriggerBot,TriggerBotPro, TriggerBotProShielded,ChargerBot, StickyBot, ExplodeyBot
...
class ModifiedEliminationGame(bs.TeamGameActivity[Player, Team]):
    ...
    def on_begin(self) -> None:
        ...
        self.botTypes = [BomberBot, BomberBotPro, BomberBotProShielded,BrawlerBot, BrawlerBotPro,BrawlerBotProShielded,TriggerBot,TriggerBotPro, TriggerBotProShielded,ChargerBot, StickyBot, ExplodeyBot]
        self.botTimer = bs.Timer(1,bs.WeakCall(self.btspawn), repeat=True)

    def btspawn(self) -> None:
        try:
            if len(self.players) == 1:
                if not self._bots.have_living_bots():
                    ...
                    if self._bots.have_living_bots():
                        if len(self.players) > 1:
...
def enable():
    elimination.EliminationGame = ModifiedEliminationGame
