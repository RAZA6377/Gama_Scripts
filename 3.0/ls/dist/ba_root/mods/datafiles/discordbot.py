...
serverlog_path = ba.env()["python_directory_user"] + "/logs/server.log"
...
async def on_ready():
    ...
    logschannel = bot.get_channel(1248724888521543691)
    try:
        await channel.purge(limit=100)
        message = await channel.send("Server Live Stats")
        logmsg = await logschannel.send("Logs")
...
async def server_logs():
    try:
        with open(serverlog_path, "r") as file:
            lines = file.readlines()
        if not lines:
            return "Empty"
        if len(lines) > 40:
...
async def update_logs():
    try:
        ...
        guild = bot.get_guild(1050804096388579380)
        channel = bot.get_channel(1248724888521543691)
        msgid = log_msg[-1]
        message = await channel.fetch_message(msgid)
        embed = discord.Embed(title="Server Logs", description=data, color=0xA020F0)
        embed.set_footer(text="Eigen.GAMA", icon_url=bot.user.avatar.url)
        embed.set_author(name="GAMA PRIVATE", icon_url=guild.icon.url)
        timestamp = datetime.datetime.utcnow()
        embed.timestamp = timestamp
...
async def before_serverlog_task():
    await bot.wait_until_ready()
